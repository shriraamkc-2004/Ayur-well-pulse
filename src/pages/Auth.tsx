...
const Auth = () => {
  ...
  const handleGoogleLogin = async (credential: string) => {
    setLoading(true);
    try {
      const response = await authAPI.googleLogin(credential, role || 'patient');
      const data = response.data;
      login(data.accessToken, data.user);
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      // tokenResponse.access_token is the access token — but we need the id_token
      // Use the Google Identity Services implicit flow instead
  ...
  // Google One Tap / Credential button uses GoogleLogin from @react-oauth/google
  // We use useGoogleLogin for more control
